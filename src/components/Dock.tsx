...
const appTitles: Record<string, string> = {
  ...
  files: 'Files', guide: 'Guide', settings: 'Settings',
}
...
const appSizes: Record<string, [number, number]> = {
  ...
  files: [640, 480], guide: [500, 560], settings: [450, 500],
}
...
export default function Dock() {
  ...
  const handleClick = (item: DockItem, e: React.MouseEvent) => {
    ...
    // Notification
    addNotif(`${item.label} launched`, item.icon === 'user' ? '👤' : item.icon === 'terminal' ? '💻' : item.icon === 'notes' ? '📝' : item.icon === 'calc' ? '🧮' : item.icon === 'music' ? '🎵' : item.icon === 'gallery' ? '🖼' : item.icon === 'browser' ? '🌐' : item.icon === 'reels' ? '📱' : item.icon === 'files' ? '📁' : '⚙️', 2000)
  }
